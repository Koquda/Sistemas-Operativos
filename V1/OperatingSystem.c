	...
	for (i=0; programList[i]!=NULL && i<PROGRAMSMAXNUMBER ; i++) {
		...
		if(PID==NOFREEENTRY){
				...
				ComputerSystem_DebugMessage(104,ERROR, 
						programList[i]->executableName, "program not valid");
		}
		else if(PID==PROGRAMDOESNOTEXIST){
				ComputerSystem_DebugMessage(104,ERROR, 
						programList[i]->executableName, "program does not exist");
		}
...
void OperatingSystem_HandleSystemCall() {
	...
	switch (systemCallID) {
		...
		case SYSCALL_END:
			...
			break;
		case SYSCALL_YIELD:
			// Falta comprobar el caso en el que tengan la misma prioridad
			if((processTable[executingProcessID].priority) == (processTable[executingProcessID + 1].priority)){
